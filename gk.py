...
def solve_sdp(Sigma, tol=1e-3, regularizer=1e-4):
    """
...
class GaussianKnockoffs:
    ...
    def __init__(self, Sigma, method="equi", mu=[], tol=1e-3, regularizer=1e-4):
        """
        Constructor
        ...
        if self.method == "equi":
            ...
            self.Ds = np.diag(solve_sdp(self.Sigma, tol=tol, regularizer=regularizer))
        else:
