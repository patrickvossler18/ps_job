...
# Load data
white_data = pd.read_csv("white_clean.csv")

factor_list = pd.read_csv("white_factor_list.csv").values.tolist()
chunk_list = pd.read_csv("white_chunk_list.csv").values.tolist()
cat_var_idx = pd.read_csv("white_cat_var_idx.csv").values.tolist()
...
# Drop Y and W
X =  white_data.drop(columns=["bweight","smoke"])


# Split train test 80-20 and save index of train data for later
X_train = X.sample(frac=0.8,random_state=200)
np.savetxt("/artifacts/white_train_msk.csv", X_train.index, delimiter=",")
